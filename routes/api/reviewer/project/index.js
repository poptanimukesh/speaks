...
router.get('/', authorize('Reviewer', '/admin/login'), function(req, res) {
  console.log('in api v1 Reviewer');
  projectFindModel.findAllByAggregated({
  }).then(function(results) {
    console.log('user');
    console.log(results);
    var result = legacyProjectHelperModel.legacyFormatWithMetaProjectsSyncReviewers(results, req.user.UserID);
    res.json({
        status: 'success',
...
router.get('/raw', authorize('Reviewer', '/admin/login'), function(req, res) {
